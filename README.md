...
- **左 rail**：品牌名、搜索、通知、**收藏的版面 / 常去版面**（数据源是站点自己那份版面
  收藏与浏览历史，详下）、本页主题 / 本页楼层、当前用户、明暗切换。宽度可拖。
  本页楼层（可直接跳楼）、当前用户、明暗切换。宽度可拖。
- **主区**：帖子 = 一串 agent 楼层。楼主正文在「用户气泡」里，回复在 agent 区，
  鼠标悬停浮出操作胶囊（引用 / 支持 / 反对 / 收藏 / 复制链接）。
- **列表页**：版面主题列表（回复数压成 `1.2k` / `3.4w`），行间按种子穿插
  `✻ Worked for 27s` 思考块和 `Running command` 工具调用行 —— 整页读起来像一份
  agent 会话日志而不是论坛帖子流。
- **右侧代码面板**：纯装饰的假代码（Rust / Python / TS / Go / Java），内容和 NGA 无关，
  项目名可配（默认 `platform`）。可切 `diff` 视图、可拖动、可整个关掉。
- **首页**：不是帖子列表，而是**版面大全** —— 按站点自己的目录**分类分组**（7 个分类 /
  659 条，含合集），带前端筛选和逐个收藏；目录外的版面归入「其它版面」；上面是
  「收藏的版面」，下面是 NGA 首页原本的头条卡片。
  「NGA 没有全局版面表」这件事和解决办法见下面「坑」第 12 条。
- **用户信息页**（`nuke.php?func=ucp&uid=N`）：资料卡 + 统计格子（发帖 / 金钱 / 声望 /
  威望 / 注册年限）+ 状态（buffs）/ 头像 / 签名 / 声望表，以及一排账号动作。
  动作按钮不重写：有 href 的当普通链接，纯 JS 动作（「更改密码」「账号关联」…）
  点下去转交给原生元素自己处理（权限、弹窗、短信验证归站点）。
- **正文**：BBSCode 渲染（`[b] [quote] [collapse] [list] [table] [s:ac:哭笑] [img]`）、
  引用升级成可折叠可跳转的卡片、NGA 官方 204 个 AC 娘/潘斯特/企鹅表情的表情面板、
  附件图（含原生异步渲染失败时的兜底）、图片灯箱 + 悬停大图预览。
...
共 6 组、24 项：外观 / 伪装 / agent 装饰 / 引用 / 楼层 / 正文图片。
「恢复默认」在面板左下角。
